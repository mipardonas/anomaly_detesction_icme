...
class HybridEncoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 anomaly_guided=False,
                 anomaly_fusion_weight=0.5,
                 anomaly_residual_weight=0.25,
        ...
        self.anomaly_guided = anomaly_guided
        self.anomaly_fusion_weight = anomaly_fusion_weight
        self.anomaly_residual_weight = anomaly_residual_weight
        ...
        self.anomaly_gate_projs = None
        self.anomaly_res_projs = None
        if self.anomaly_guided:
            self.anomaly_gate_projs = nn.ModuleList([
                nn.Sequential(
            ...
            self.anomaly_res_projs = nn.ModuleList([
                nn.Sequential(
                    nn.Conv2d(1, hidden_dim, kernel_size=3, padding=1, bias=False),
                    nn.BatchNorm2d(hidden_dim),
                ) for _ in range(len(in_channels))
    ...
    def forward(self, feats, anomaly_maps=None):
        assert len(feats) == len(self.in_channels)
        proj_feats = [self.input_proj[i](feat) for i, feat in enumerate(feats)]

        if self.anomaly_guided and anomaly_maps is not None:
            if anomaly_maps.dim() == 3:
                anomaly_maps = anomaly_maps.unsqueeze(1)
            anomaly_maps = anomaly_maps.to(dtype=proj_feats[0].dtype, device=proj_feats[0].device)
            if self.anomaly_map_minmax_norm:
                a_min = anomaly_maps.amin(dim=(-2, -1), keepdim=True)
                a_max = anomaly_maps.amax(dim=(-2, -1), keepdim=True)
                anomaly_maps = (anomaly_maps - a_min) / (a_max - a_min + 1e-6)

            fused_proj_feats = []
            for i, feat in enumerate(proj_feats):
                anomaly_at_level = F.interpolate(
                    anomaly_maps,
                    size=feat.shape[-2:],
                    mode='bilinear',
                    align_corners=False,
                )
                anomaly_residual = self.anomaly_res_projs[i](anomaly_at_level)
                anomaly_gate = self.anomaly_gate_projs[i](anomaly_at_level)
                fused_feat = feat + self.anomaly_residual_weight * anomaly_residual
                fused_feat = fused_feat * (1.0 + self.anomaly_fusion_weight * anomaly_gate)
                fused_proj_feats.append(fused_feat)
            proj_feats = fused_proj_feats
