...
class BatchImageCollateFunction(BaseCollateFunction):
    ...
    def apply_mixup(self, images, targets):
        ...
        Returns:
            tuple: Updated images, targets, and anomaly maps
        """
        anomaly_maps = None
        if targets and 'anomaly_map' in targets[0]:
            anomaly_maps = torch.stack([torch.as_tensor(t.pop('anomaly_map')).float() for t in targets], dim=0)
        ...
        if random.random() < self.mixup_prob and self.mixup_epochs[0] <= self.epoch < self.mixup_epochs[-1]:
            ...
            if anomaly_maps is not None:
                anomaly_maps = anomaly_maps.roll(shifts=1, dims=0).mul_(1.0 - beta).add_(anomaly_maps.mul(beta))
        ...
        return images, targets, anomaly_maps
    ...
    def __call__(self, items):
        ...
        # Mixup
        images, targets, anomaly_maps = self.apply_mixup(images, targets)
        ...
        if self.scales is not None and self.epoch < self.stop_epoch:
            ...
            if anomaly_maps is not None:
                anomaly_maps = F.interpolate(anomaly_maps, size=sz, mode='bilinear', align_corners=False)
