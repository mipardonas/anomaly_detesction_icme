...
class Compose(T.Compose):
    ...
    def _apply_transform(transform, sample):
        # Keep anomaly maps synchronized with geometric transforms only.
        # Exclude them from instance-indexing transforms like SanitizeBoundingBoxes
        # and from image-only transforms like RandomPhotometricDistort / ConvertPILImage.
        if len(sample) > 1 and isinstance(sample[1], dict) and 'anomaly_map' in sample[1] \
            and type(transform).__name__ not in Compose._ANOMALY_GEOMETRIC_TRANSFORMS:
            target = sample[1]
            anomaly_map = target.pop('anomaly_map')
            try:
                sample = transform(sample)
            finally:
                if len(sample) > 1 and isinstance(sample[1], dict):
                    sample[1]['anomaly_map'] = anomaly_map
            return sample
