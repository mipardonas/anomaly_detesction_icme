...
class Mosaic(T.Transform):
    ...
    def create_mosaic_from_cache(self, mosaic_samples, max_height, max_width):
        ...
        for i, sample in enumerate(mosaic_samples):
            ...
            if 'anomaly_map' in target:
                if merged_anomaly_map is None:
                    device = target['anomaly_map'].device
                    dtype = target['anomaly_map'].dtype
                    merged_anomaly_map = torch.zeros((1, max_height * 2, max_width * 2), dtype=dtype, device=device)
                top, left = placement_offsets[i][1], placement_offsets[i][0]
                h, w = target['anomaly_map'].shape[-2:]
                merged_anomaly_map[:, top:top + h, left:left + w] = target['anomaly_map']
        ...
        if merged_anomaly_map is not None:
            merged_target['anomaly_map'] = merged_anomaly_map
    ...
    def create_mosaic_from_dataset(self, images, targets, max_height, max_width):
        ...
        for i, img in enumerate(images):
            merged_image.paste(img, placement_offsets[i])
            if 'anomaly_map' in targets[i]:
                if merged_anomaly_map is None:
                    device = targets[i]['anomaly_map'].device
                    dtype = targets[i]['anomaly_map'].dtype
                    merged_anomaly_map = torch.zeros((1, max_height * 2, max_width * 2), dtype=dtype, device=device)
                top, left = placement_offsets[i][1], placement_offsets[i][0]
                h, w = targets[i]['anomaly_map'].shape[-2:]
                merged_anomaly_map[:, top:top + h, left:left + w] = targets[i]['anomaly_map']
    ...
    def forward(self, *inputs):
        ...
        if 'anomaly_map' in mosaic_target:
            mosaic_target['anomaly_map'] = convert_to_tv_tensor(mosaic_target['anomaly_map'], 'masks')
