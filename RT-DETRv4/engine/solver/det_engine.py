...
def train_one_epoch(self_lr_scheduler, lr_scheduler, model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for i, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        if len(batch) == 3:
            samples, targets, anomaly_maps = batch
            anomaly_maps = anomaly_maps.to(device)
        else:
        ...
        if scaler is not None:
            ...
            outputs = model(samples, targets=targets,
                            teacher_encoder_output=teacher_encoder_output_for_distillation,
                            anomaly_maps=anomaly_maps) # NEW kwarg
            loss_dict = criterion(outputs, targets, **metas)
...
def evaluate(model: torch.nn.Module, criterion: torch.nn.Module, postprocessor, data_loader, coco_evaluator: CocoEvaluator, device):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        outputs = model(samples, anomaly_maps=anomaly_maps)
