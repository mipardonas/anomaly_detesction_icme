    ...
    def load_resume_state(self, path: str):
        ...
        if path.startswith('http'):
            ...
            if checkpoint_path.stat().st_size == 0:
                raise RuntimeError(
                    f"Resume checkpoint is empty: {checkpoint_path}. "
                    "This usually means the checkpoint save was interrupted."
                )
            try:
                state = torch.load(path, map_location='cpu')
            except EOFError as exc:
                raise RuntimeError(
                    f"Failed to load checkpoint {checkpoint_path}: file is truncated or corrupted."
                ) from exc
