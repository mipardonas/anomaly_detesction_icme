...
def apply_anomaly_mode(cfg, anomaly_mode):
    ...
    for loader_name in ['train_dataloader', 'val_dataloader']:
        loader_cfg = cfg.yaml_cfg.get(loader_name, None)
        ...
        if dataset_cfg is None:
            continue
        if enable_anomaly:
            dataset_cfg.setdefault('anomaly_map_use_basename_only', True)
        else:
            dataset_cfg['anomaly_map_dir'] = None
...
if __name__ == '__main__':
    ...
    parser.add_argument('--anomaly-mode', type=str, choices=['on', 'off'],
                        help='enable or disable anomaly-map input/fusion without editing the YAML')
